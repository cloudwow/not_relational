...
=== Rails configuration
1. NonRelational will look for it's configuration in config/database.yml .
2. If you will not be using ActiveREcord, turn it off.
3. Add the following to application.rb (or application_controller.rb)
...
=== Encryption
1. Encrypt the whole model
2. Encrypt individual property
3. Remove encryption form individual property.
=== Unit tests
1. MemoryRepository for fast or offline tests.
== Copyright
